...
// ICP Scan Matching algorithm for Pose Transform Estimatiom /LOcalisation
...
int main()
{
	...
	lidar_bp.SetAttribute("rotation_frequency", "25");
	lidar_bp.SetAttribute("points_per_second", "200000");
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
          ...
          std::cout<<"Processing Scan : "<<n_scans <<std::endl;
		  ...
		  // Find pose transform by using ICP matching
          ...
          // TODO: Transform scan so it aligns with ego's actual pose and render that scan
		   ...
		   pcl::transformPointCloud (*cloudFiltered, *transformed_scan, transform);
			...
			if(maxError > 3.0 || distDriven >= 170.0 ){
